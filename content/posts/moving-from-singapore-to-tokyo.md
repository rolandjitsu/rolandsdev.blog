...
Everything my team builds - services, CLIs - is Rust. (The rest of the company lives in C++, Go, and Python.) A lot of that choice came down to a hard constraint: some of the software we built on my team often had to run on the Mujin controllers, where CPU and memory are scarce (reserved for critical systems), and Rust's low footprint made it an easy call.
